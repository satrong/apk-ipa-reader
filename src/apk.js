import { ApkManifestReader, parseApkinit } from './lib.js';

var manifest, resources, fileinfo = {};
 ...
 */
export default function (entries, callback) {
    ...
    if (manifest && resources) {
        manifest.getData(new zip.BlobWriter(manifest), function (blob1) {
            ApkManifestReader(blob1, function (fileAttr, err) {
                if (err) {
                    callback('解析文件出错了 ' + err);
                } else {
                    resources.getData(new zip.BlobWriter(resources), function (blob2) {
                        parseApkinit(blob2, fileAttr, function (attrs) {
